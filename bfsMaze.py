...
def BFS(dict, start, goal):
    ...
    while len(nonVisited) > 0:
        # Get the first Inserted node(FIFO Implementation)
        currentNode = nonVisited.pop(0)
...
def main():
    ...
    # Close File
    file.close()
    # Finding path using BFS for pacman
    path = BFS(dict, start, goal)
    print()
